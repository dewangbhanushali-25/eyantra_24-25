...
class aruco_tf(Node):
    def __init__(self):
        ...
        # Passing service to pass the box onto the ebot
        self.passing_srv = self.create_service(PassingSw,'passing_srv',self.pass_callback, callback_group=self.callback_group)
        ...
        self.ee_pos_timer = self.create_timer(0.1, self.get_pose_ee)
        ...
        self.joint_names = []
        self.joint_angles = []
        self.car_present = False # Variable to check if the ebot is in front of the arm or not
    ...
    def process_image(self):
        ...
        # print(self.car_present, box_list)
        if self.car_present:
            if self.current_state == 'start_point':
                # self.move_to_marker(self.make_pose([0.081504, 0.10869, 0.52004],[0.71016, 0.70377, -0.010568, -0.016729]))
                # self.move_to_marker(self.make_pose([0.16836, 0.10761, 0.54998],[0.70648, 0.70057, -0.067666, -0.074264]))
                self.move_to_marker(self.make_pose([0.16324, 0.11132, 0.59002],[-0.52517, 0.85084, 0.0081286, 0.014236])) # og pose
                ...
                self.move_to_marker(self.make_pose([0.43936, 0.105, 0.55783], [0.70995, 0.70361, -0.01791, -0.024119])) # Anees
                ...
                self.move_to_marker(self.make_pose([0.53108, 0.075126, -0.049006], [0.70995, 0.70361, -0.01791, -0.024119])) # Anees
    ...
    def move_to_marker(self, target_pose):
        ...
        if self.current_state == 'move_to_drop':
            position_threshold = 0.65 # Larger threshold for linear movement in the 'move_to_drop' state
            orientation_threshold = 0.65 # Larger threshold for orientation
        elif self.current_state == 'start_point':
            position_threshold = 0.35 # 3 cm threshold for linear movement
            orientation_threshold = 0.35  # Smaller threshold for orientation
        elif self.current_state == 'moving_to_box':
            position_threshold = 0.1 # 3 cm threshold for linear movement
            orientation_threshold = 3.5  # Larger threshold for orientation
        elif self.current_state == 'move_to_intermediate':
            position_threshold = 1.0 # 3 cm threshold for linear movement
        ...
        linear_velocity_gain = 30.0  # Gain for linear control
        max_linear_velocity = 75.0 # Maximum linear speed limit
        ...
        if abs(angle) > orientation_threshold:  # Threshold for alignment
            angular_velocity_gain = 30.0 * (angle / math.pi)  # Scales down as angle decreases
            axis = [
    ...
    def gripper_magnet_off(self, box_name):
        ...
        future = client.call_async(request)
        future.add_done_callback(partial(self.magnet_off_callback,box_name=box_name))

    def magnet_off_callback(self, future, box_name):
        try:
            response = future.result()
            self.magnet_off = response.success
            print(f"Magnet is off {self.magnet_off}")
            if self.magnet_off:
                print("Box will be deleted from the list")
    ...
    def pass_callback(self, request, response):
        """
        This function is the callback function for the passing service.
        The idea is to move the ebot near the arm, once the bot is near the arm its aruco marker must be detected
        Once the marker has been detected the arm must move to the box, grab it and drop it onto the bot. Once the box has been dropped
        the response must show that the request has been successfully completed. Once the box is on the bot, it must continue it journey 
        do the docking station.
        """
        self.car_present = request.get_box

        self.get_logger().info("Waiting for box")

        rate = self.create_rate(2, self.get_clock())
        while not self.box_dropped:
            self.get_logger().info("Waiting for box to be dropped...")
            rate.sleep()
        ...
        # self.box_dropped = False
        print(f"Box dropped, response sent {response.success}")
        return response
