    ...
    if detected_ids is not None:
        ...
        for i, marker_id in enumerate(detected_ids):
            ...
            if area > aruco_area_threshold:  # Threshold to filter out far markers
                ...
                # aruco_pose = get_pose(rvec=rvec, tvec=tvec[0]) # we get the pose of each box 
                # pose.append(aruco_pose)

                # print(marker_id, aruco_pose)
...
class aruco_tf(Node):
    def __init__(self):
        ...
        self.ee_pos_timer = self.create_timer(0.002, self.get_pose_ee)
    ...
    def get_pose_ee(self):
        try:
            ...
            # Print the position and orientation for debugging purposes
            # self.get_logger().info(f"End Effector Position (wrist_3 wrt base_link): x={pose.position.x}, y={pose.position.y}, z={pose.position.z}")
            # self.get_logger().info(f"Orientation: x={pose.orientation.x}, y={pose.orientation.y}, z={pose.orientation.z}, w={pose.orientation.w}")
    ...
    def process_image(self):
            ...
            if len(center_aruco_list[i]) >= 2 and len(distance_from_rgb_list) > i:
              ...
              # Publish the final transform from base_link to obj_<marker_id>
                ...
                if not key in self.aruco_poses:
                    ...
                    # print(marker_id, marker_pose)
                # self.move_to_marker(self.aruco_poses[0])
        ...
        cv2.waitKey(1)
        # self.move_to_marker(self.make_pose([-0.107, 0.164, 0.467] , [0.008, 0.708, 0.706, 0.002] ))
        # self.move_to_marker(self.aruco_poses[0])

        # if self.magnet_on:
        #     self.move_to_marker(self.make_pose([0.16194, 0.10803, 0.46418] , [0.50503, 0.49614, 0.4992, 0.49959] ))
        #     self.move_to_marker(self.make_pose([0.16384, 0.10834, 0.46911] , [0.50482, 0.49599, 0.49938, 0.49977] )) # init pos
        ...
        if self.current_state == 'start_point':
            # self.move_to_marker(self.make_pose([0.16377, 0.10833, 0.46896],[0.71035, 0.70368, 0.013332, 0.0074897]))
            ...
            self.move_to_marker(self.make_pose([0.16144, 0.10782, 0.52099],[0.7103, 0.70369, 0.014805, 0.0089696]))
            ...
            if len(box_list) > 0:
                ...
                # if current_box_pose.position.y < 0.0: # condiiton for box on the left of the arm
                #     print(box_list)
                #     # # self.move_to_marker(self.make_pose([0.098198, -0.17024, 0.46944] , [0.70712, -0.027751, -0.021734, 0.70621] ))
                ...
                #     # self.move_to_marker(self.make_pose([0.13511, -0.040487, 0.53689] , [-0.16588, 0.98348, 0.0036838, 0.072344] ))
                #     self.move_to_marker(self.aruco_poses[self.current_box])
                #     # [0.16377, 0.10833, 0.46896] [0.71035, 0.70368, 0.013332, 0.0074897]
                #     # self.move_to_marker(self.make_pose([0.16377, 0.10833, 0.46896],[0.71035, 0.70368, 0.013332, 0.0074897]))
            ...
            if self.reached_drop:
                ...
                if self.reached_drop:
                    self.current_state = 'start_point'
                    self.reached_drop = False
                    # self.aruco_poses.pop(self.current_box)
                # self.box_dropped = False
    ...
    def move_to_marker(self, target_pose):
        ...
        if self.current_state == 'move_to_drop':
            position_threshold = 0.3 # Larger threshold for linear movement in the 'move_to_drop' state
            orientation_threshold = 0.5  # Larger threshold for orientation
        elif self.current_state == 'start_point':
            position_threshold = 0.2  # 3 cm threshold for linear movement
            orientation_threshold = 0.23  # Smaller threshold for orientation
        elif self.current_state == 'moving_to_box':
            position_threshold = 0.03 # 3 cm threshold for linear movement
        ...
        linear_velocity_gain = 11.0  # Gain for linear control
        max_linear_velocity = 6.0  # Maximum linear speed limit
        ...
        max_angular_velocity = 7.0 # Maximum angular speed limit
        if abs(angle) > orientation_threshold:  # Threshold for alignment
            angular_velocity_gain = 107.0 * (angle / math.pi)  # Scales down as angle decreases
            axis = [
...
def main(args=None):
    ...
    # # aruco_tf_node.move_to_marker(aruco_tf_node.aruco_poses[0])
